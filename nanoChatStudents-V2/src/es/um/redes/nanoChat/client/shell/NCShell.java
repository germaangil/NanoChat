...
public class NCShell {
	...
	private String[] readChatCommandFromStdIn(NCConnector ncclient) {
		...
		while (true) {
			...
			{
				StringBuffer privMessage = new StringBuffer();
				while (st.hasMoreTokens())
					privMessage.append(st.nextToken()+" ");
				vargs.add(privMessage.toString());
				break;
			}
			default:
	...
	private boolean validateCommandArguments(String[] args) {
		...
		case NCCommands.COM_RENAME:
			if (args.length == 0 || args.length > 1) {
				System.out.println("Correct use: rename <name>");
				return false;
			}
			break;
		//private requiere los parámetros <nickname> <message>
		case NCCommands.COM_PRIVATE:
			if (args.length == 0) {
